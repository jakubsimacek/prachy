...
function processActionSteps(funcArray, flowData) {
	dbg('processActionSteps invoked')
  return _.reduce(funcArray, (promise, func) => {
    return promise.then((previousResult) => {
			  flowData.actualStep = func.name
			  dbg(`Processing step ${func.name}`)
			  return Promise.resolve(func(flowData, previousResult))
      })
...
function findAkceInArray(arr, akce) {
  return _action = _.find(arr, (act) => {
    return _.find(act.aliases, (item) => { return item == akce })
  })
}
...
module.exports.generalGetController = function (req, res) {
	dbg('General controller invoked')
  const _requestPath = parseParameters(req)
	const _entityConfigPath = getEntityConfigurationPath(_requestPath)
	const _entityToWorkWith = _.takeRight(_entityConfigPath.path)[0]
	const _actionConfig = getActionConfig(_entityToWorkWith, _requestPath.akce)
  const _handlerGenerator = _actionConfig.handlerGenerator
	//dbg(_entityToWorkWith)
	const _hasLastId = ! _.isUndefined(_entityToWorkWith.id)
  const _flowData = {
		express: { req: req, res: res },
		requestPath: _requestPath,
		entityConfigPath: _entityConfigPath,
		entityToWorkWith: _entityToWorkWith,
		actionConfig: _actionConfig,
    hasLastId: _hasLastId
	}
  const _handlerSteps = cfg.getHandlerSteps(_handlerGenerator, 'get', _requestPath.path.length, _hasLastId)
  processActionSteps(_handlerSteps, _flowData).catch((err) => {
